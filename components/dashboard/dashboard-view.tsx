...
export function DashboardView() {
  const {
    ...
    queryFn: () => apiClient.get<DashboardSummary>("/api/v1/reports?type=summary"),
  });
  ...
  const stats = [
    {
      label: "Total Revenue",
      value: `Rp ${new Intl.NumberFormat("id-ID").format(summary.totalRevenue ?? 0)}`,
      change: `${(summary.revenueChange ?? 0) > 0 ? "+" : ""}${(summary.revenueChange ?? 0).toFixed(1)}%`,
      icon: DollarSign,
    ...
    },
    {
      ...
      change: `${(summary.ordersChange ?? 0) > 0 ? "+" : ""}${(summary.ordersChange ?? 0).toFixed(1)}%`,
      icon: ShoppingBag,
    ...
    },
    {
      ...
      value: `Rp ${new Intl.NumberFormat("id-ID").format(summary.avgOrderValue ?? 0)}`,
      change: "0.0%",
      icon: TrendingUp,
      trend: "up",
    },
